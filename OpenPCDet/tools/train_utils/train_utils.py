...
def train_one_epoch(model, optimizer, train_loader, model_func, lr_scheduler, accumulated_iter, optim_cfg,
    ...
    for cur_it in range(start_it, total_it_each_epoch):
        ...
        if rank == 0:
            ...
            # print("batch:{}".format(batch['frame_id']))
            # print("batch:{}".format(batch['spatial_features'].shape))


            # file = '/data/feature_kitti_train/{}.txt'.format(batch['frame_id'][0])
            # with open(file, 'w') as f: 
            #     json.dump(batch['spatial_features'].tolist(), f)
            ...
            # print("loss:")
            # print(loss.item())
            # output_dict['id_list'].append(batch['frame_id'][0])
            # output_dict['loss_list'].append(loss.item())
...
#this piece of code will not update model
# def train_one_epoch(model, optimizer, train_loader, model_func, lr_scheduler, accumulated_iter, optim_cfg,
#                     rank, tbar, total_it_each_epoch, dataloader_iter, tb_log=None, leave_pbar=False, 
#                     use_logger_to_record=False, logger=None, logger_iter_interval=50, cur_epoch=None, 
#                     total_epochs=None, ckpt_save_dir=None, ckpt_save_time_interval=300, show_gpu_stat=False, use_amp=False):
...
#             # print("loss:")
#             # print(loss.item())
#             # print('frame id:{}'.format(batch['frame_id'][0]))
#             output_dict['id_list'].append(batch['frame_id'][0])
#             output_dict['loss_list'].append(loss.item())
...
#         # if cur_it % 1000 == 0 or cur_it == total_it_each_epoch - 1:
#         #     file_name = 'output_nuscenes_loss/' + str(cur_it) + '.txt'
#         #     with open(file_name, 'w') as f:
#         #         json.dump(output_dict, f)
#         #         output_dict = {'loss_list':[], 'id_list':[]}

#     with open('waymo_output/waymo_loss.txt', 'w') as f:
#         json.dump(output_dict, f)
#     if rank == 0:
#         pbar.close()
#     return accumulated_iter
