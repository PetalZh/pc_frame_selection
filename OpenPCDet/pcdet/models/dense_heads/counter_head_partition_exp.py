...
class CounterHeadPartitionExp(nn.Module):
    ...
    def forward(self, data_dict):
        ...
        # ranges = [[0, 42, 0, 42], [42, 84, 0, 42], [84, 128, 0, 42], [0, 42, 42, 84], [42, 84, 42, 84], [84, 128, 42, 84], [0, 42, 84, 128], [42, 84, 84, 128], [84, 128, 84, 128]]
        # ranges = [[0, 64 + expanding, 0, 128], [64-expanding, 128, 0, 128]]
        # ranges = [[0, 64, 0, 128], [64, 128, 0, 128]]
        pred_dicts = []
        ...
        if not self.training or self.predict_boxes_when_training:
            pred_dicts = self.forward_ret_dict['pred_dicts']
            # print('len of pred_dicts: {}'.format(len(pred_dicts)))
            ...
            for idx, pred_dict in enumerate(pred_dicts):
                ...
                # print('len of pred_dict {}'.format(len(pred_dict)))
                ...
                # file = '/data/hm_partition2_overlap0.3/{}.txt'.format(self.progress)
                # with open(file, 'w') as f: 
                #     json.dump(hm_list, f)

                # file = '/data1/hms_partition_ep10/gt_{}.txt'.format(self.progress)
                # with open(file, 'w') as f: 
                #     json.dump(gt_list, f)
