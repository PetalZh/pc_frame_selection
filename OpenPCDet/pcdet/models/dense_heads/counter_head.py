...
class CounterHead(nn.Module):
    def __init__(self, model_cfg, input_channels, num_class, class_names, grid_size, point_cloud_range, voxel_size,
        ...
        # print('class name each head: ')
        # print(self.model_cfg.CLASS_NAMES_EACH_HEAD)
    ...
    def forward(self, data_dict):
        ...
        # file = '/data/feature_waymo/{}.txt'.format(self.progress)
        ...
        if not self.training or self.predict_boxes_when_training:
            ...
            #     mask = target_dicts['masks'][idx]
            #     batch_gt_count = torch.sum(mask, dim=1, keepdim=True)
            ...
            #     data_dict['center_count'] = center_count

            #     if obj in result.keys():
            #         result[obj]['gt_count'].append(gt_count.item())
            #         result[obj]['center_count'].append(center_count.item())
            #         # result[obj]['hm'].append(pred_dict['hm'].toList())
            #     else:
            #         result[obj] = {'gt_count': [gt_count.item()], 'center_count': [center_count.item()]}

            #     # file = '/data/waymo_hm/{}.txt'.format(self.progress)
            #     # with open(file, 'w') as f: 
